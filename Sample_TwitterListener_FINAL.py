...
class Sample(tweepy.Stream):   
    ...
    def __init__(self, consumerKey, consumerSecret, accessToken, accessTokenSecret, tweet_count):
    ...
    def on_status(self, status):   
        ...
        # add logic to close the connection after a certain number of tweets are retrieved
        # otherwise the connection will stay open indefinitely or until there is an issue 
        # that cause it to close since it's not a guaranteed pipe 
        ...
        #to do: add feature to remove tweets with sensitive content, e.g. videos or photos 

        # acquiring location, converting coordinates to string as it's easier to handle 
        ...
        # comment out when using the earlier code to reject all tweets with profanity 
        ...
        # increment the tweet counter and the counter for printing every nth tweet 
    ...
    # doesn't have a limit. However, there are limits as to the number of attempts to
    ...
    # the flag is meant to show a custom message instead of the "Twitter Disconnected" message
    # needed to override the "on_disconnect()" to make that happen. 
