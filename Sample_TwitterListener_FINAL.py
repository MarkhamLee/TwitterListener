...
class Sample(tweepy.Stream):
    ...
    def on_status(self, status: object):
        ...
        # add logic to close the connection after a certain number of
        # tweets are retrieved otherwise the connection will stay open
        # indefinitely or until there is an issue that cause it to close
        # since it's not a guaranteed pipe
        ...
        # to do: add feature to remove tweets with sensitive content,
        ...
        # Commented out, but the feature to reject tweets containing profanity
        # is here if desired
        ...
        # Polarity a score between -1 and 1, where -1 is negative sentiment
        # and +1 is positive subjectivity, which lies beteen 0 and 1
        blob = TextBlob(tweet_text)
        tweet_sentiment = blob.sentiment
        polarity = tweet_sentiment.polarity
        subjectivity = tweet_sentiment.subjectivity

        # Vader sentiment analysis, which returns three probabilities for a
        # given sentence being positive, negative or neutral, with all the
        # probabilities adding up to 1 E.g., a sentence scores 0.24 for
        # negative, 0.7 for positive and 0.06 for neutral a compound score
        # is also generated that is the overall score the sentence Vader is
        # optimized for social media, so it "should" give better sentiment
        # scores for Twitter.
        ...
        # has been calculated comment out when using the earlier code to reject
        # all tweets with profanity
        ...
        db = dataset.connect("sqlite:///sampletweetsdb.db")

        table = db['sample_tweet_table_two']
        table.insert(dict(
            text=tweet_text,
            ...
            text_blob_subjectivity=subjectivity,
    ...
    # ensures a more elegant disconnect sequence
    # the flag is meant to show a custom message instead of the
    # "Twitter Disconnected" message needed to override the
    # "on_disconnect()" to make that happen.
